...
from cauchy import n_cauchy


class CauchyFittingN(FittingLsq):
    def __init__(self, bounds=(), p0='Automatic', known=()):
        super(CauchyFittingN, self).__init__(bounds=bounds, p0=p0)
        self.mean = None
        self.var = None
        self.known = known
    ...
    def _set_p0(self, n, ks):
        ...
        if isinstance(self.p0, str):
            if self.p0.upper() == 'AUTOMATIC':
                for k in ks:
                    for _id, _label in enumerate(['a', 'g', 'x0']):
                        if _label in k:
                            p0[j] = params[_id]
                            j += 1
        else:
    ...
    def _set_func(self, n):
        return n_cauchy(n, known=self.known)
